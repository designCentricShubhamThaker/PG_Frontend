...
const AddNewPumpChild = ({
  ...
  const handleSave = async () => {
    try {
      ...
      setError(err.message || 'Failed to save pump');
    } finally {
    ...
    <Dialog open={isOpen} onClose={onClose} className="relative z-10" >
      ...
      <div className="fixed inset-0 overflow-y-auto">
        <div className="flex min-h-full items-center justify-center p-4 ">
          <DialogPanel className="w-full max-w-4xl transform overflow-hidden rounded-lg bg-white shadow-xl transition-all p-4">
            ...
            <DialogTitle as="h3">
              <div className="bg-[#FF6701] p-4 rounded-t-md border-b border-orange-200 shadow-sm text-center">
                <h3 className="text-white text-xl font-bold flex tracking-wide gap-2">
                  {editingBottle ? 'Edit Pump' : 'Create New Pump'}
                </h3>
              </div>
            </DialogTitle>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    {editingBottle ? 'Update' : 'Create'} Pump
                  </>
                )}
              </button>
            </div>
...
export default AddNewPumpChild;