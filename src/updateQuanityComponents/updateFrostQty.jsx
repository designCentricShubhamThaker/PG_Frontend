...
const UpdateFrostQty = ({ isOpen, onClose, orderData, itemData, onUpdate }) => {
  ...
  useEffect(() => {
    if (isOpen && itemData?.team_assignments?.frosting) {
      setAssignments(itemData.team_assignments.frosting.map(assignment => ({
        ...assignment,
        todayQty: 0,
        notes: ''
      })));
  ...
  const preserveGlassItemDetails = (frostingAssignment, allGlassAssignments) => {
    const glassItemId = frostingAssignment.glass_item_id?._id || frostingAssignment.glass_item_id;
