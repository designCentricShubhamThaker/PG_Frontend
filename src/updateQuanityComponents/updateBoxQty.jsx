...
const UpdateBoxQty = ({ isOpen, onClose, orderData, itemData, onUpdate }) => {
    ...
    useEffect(() => {
        if (isOpen && itemData?.team_assignments?.boxes) {
            setAssignments(itemData.team_assignments.boxes.map(assignment => ({
                ...assignment,
                todayQty: 0,
                notes: ''
            })));
    ...
    const updateLocalStorageWithOrder = (updatedOrder) => {
        try {
            ...
            const existingOrders = getOrdersFromLocalStorage(orderType, TEAMS.BOXES);
            ...
            saveOrdersToLocalStorage(existingOrders, orderType, TEAMS.BOXES);
    ...
    const handleSave = async () => {
        try {
            ...
            // const response = await axios.patch('https://pg-backend-o05l.onrender.com/api/boxes', {
            //     orderNumber: orderData.order_number,
            //     itemId: itemData._id,
            //     updates
            // });
            const response = await axios.patch('http://localhost:5000/api/boxes', {
                orderNumber: orderData.order_number,
                itemId: itemData._id,
                updates
            });
                    ...
                    <DialogPanel className="w-full max-w-7xl transform overflow-hidden rounded-lg bg-white shadow-xl transition-all p-4">
                        ...
                        <div className="bg-gradient-to-r from-orange-800 via-orange-600 to-orange-400 px-4 py-3 mt-6 rounded-md">
                            <div className="grid gap-4 text-white font-semibold text-sm items-center"
                                ...
                                <div className="text-left">Box Name</div>
                                <div className="text-left">Approval Code</div>
                                ...
                                return (
                                    <div key={assignment._id}
                                        ...
                                        <div className="grid gap-4 text-sm items-center"
                                            style={{
                                                gridTemplateColumns: '2fr 1fr 1fr 1fr 3fr 2fr '
                                            }}>

                                            <div className="text-left">
                                                <div className="font-medium text-orange-900">{assignment.box_name}</div>
                                                <div className="text-xs text-gray-600">
                                                    {assignment.decoration} #{assignment.decoration_no}
                                                </div>
                                            </div>
                                            <div className="text-left">
                                                <div className="font-medium text-orange-900">{assignment.approval_code}</div>
